...
/* =========================
   الإدارات
========================= */
electron_1.ipcMain.handle("departments:list", () => {
...
electron_1.ipcMain.handle("departments:create", (_event, data) => {
    const db = (0, db_1.getDb)();
...
electron_1.ipcMain.handle("departments:update", (_event, data) => {
    const db = (0, db_1.getDb)();
    const stmt = db.prepare(`
    UPDATE departments
    SET name = @name,
...
electron_1.ipcMain.handle("departments:delete", (_event, id) => {
    const db = (0, db_1.getDb)();
...
/* =========================
   الموظفين
========================= */
electron_1.ipcMain.handle("employees:list", (_event, filters = {}) => {
    const db = (0, db_1.getDb)();
    let sql = `
    SELECT 
      e.*,
...
electron_1.ipcMain.handle("employees:create", (_event, data) => {
    const db = (0, db_1.getDb)();
    const stmt = db.prepare(`
...
electron_1.ipcMain.handle("employees:update", (_event, data) => {
    const db = (0, db_1.getDb)();
    const stmt = db.prepare(`
    UPDATE employees
...
electron_1.ipcMain.handle("employees:delete", (_event, id) => {
    const db = (0, db_1.getDb)();
    const used = db.prepare(`
    SELECT COUNT(*) AS count 
...
/* =========================
   التقييمات الشهرية
========================= */
electron_1.ipcMain.handle("evaluations:load-month", (_event, data) => {
    const db = (0, db_1.getDb)();
    return db.prepare(`
    SELECT 
      e.id AS employee_id,
...
electron_1.ipcMain.handle("evaluations:save-bulk", (_event, data) => {
    const db = (0, db_1.getDb)();
...
electron_1.ipcMain.handle("evaluations:copy-month", (_event, data) => {
    const db = (0, db_1.getDb)();
...
/* =========================
   التقارير
========================= */
electron_1.ipcMain.handle("reports:employee-period", (_event, data) => {
    const db = (0, db_1.getDb)();
    const months = [];
